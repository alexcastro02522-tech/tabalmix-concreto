...
if status_atual != "Ativo":
  st.markdown(
      """
        <div style="max-width: 650px; margin: 60px auto; background: #0f172a; padding: 40px; border-radius: 16px; border: 2px solid #ef4444; text-align: center; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.5);">
            <span style="font-size: 50px;">🔒</span>
            <h1 style="color: #ef4444 !important; margin-top: 10px;">Acesso Suspenso / Assinatura Pendente</h1>
            <p style="color: #cbd5e1; font-size: 16px; margin-top: 15px;">
                O período da sua licença encerrou. Realize o pagamento instantâneo via Pix ou Cartão para liberar o acesso:
            </p>
        </div>
    """,
      unsafe_allow_html=True,
  ...
  with col_p1:
    if st.button("💳 Pagar Plano Mensal (R$ 250,00)"):
      try:
        ...
        st.success(
            f"🔗 [Clique aqui para abrir o pagamento seguro]"
            f"({checkout_url})"
        )
      except Exception as e:
        st.error(
            f"Erro ao gerar pagamento com Mercado Pago. Verifique o token."
            f" Detalhe: {e}"
        )
    ...
    if st.button("🌟 Pagar Plano Anual (R$ 2.400,00)"):
      try:
        ...
        preference_data = {
            "items": [{
                "title": (
                    "Tabalmix Concreto - Assinatura Anual de Sistema de Gestão"
                ),
                "quantity": 1,
                "unit_price": 2400.0,
                "currency_id": "BRL",
        ...
        st.error(f"Erro ao gerar pagamento via Mercado Pago: {e}")

  st.markdown(
      f"""
        <div style="max-width: 650px; margin: 20px auto; background: #1e293b; padding: 20px; border-radius: 10px; border: 1px solid #334155; text-align: center;">
            <p style="margin: 0; color: #94a3b8; font-size: 14px; text-transform: uppercase;">Ou transfira via Chave Pix Manual:</p>
            <h3 style="color: #38bdf8 !important; margin: 10px 0 0 0; font-family: monospace; font-size: 18px;">{chave_pix_recebimento}</h3>
        </div>
    """,
      unsafe_allow_html=True,
  )
  ...
  if menu == "📊 Visão Geral":
    ...
    if not df_veiculos.empty:
      ...
      st.info(
          "Nenhum equipamento cadastrado. Utilize o menu lateral para"
          " cadastrar na aba 'Frota e Maquinários'."
      )
    ...
    df_v = pd.read_sql(
        "SELECT codigo_patrimonio, modelo FROM veiculos", conn
    )
    if df_v.empty:
      ...
      with st.form("form_combustivel", clear_on_submit=False):
        ...
        with col1:
          ...
          litros = st.number_input(
              "Quantidade de Litros Abastecidos", min_value=0.1, value=100.0, format="%.2f"
          )
        ...
        if salvar_comb:
          cursor.execute(
              "INSERT INTO combustivel (equipamento, litros, valor_total, km_horimetro, posto_posto, motorista, data) VALUES (?, ?, ?, ?, ?, ?, ?)",
              (
                  equipamento_comb,
                  float(litros),
                  float(valor_total),
    ...
    if df_v.empty:
      st.warning(
          "Cadastre equipamentos na frota antes de registrar mobilizações."
      )
    else:
    ...
    if df_v.empty:
      st.warning(
          "Cadastre pelo menos um equipamento antes de abrir uma Ordem de"
          " Serviço."
      )
    else:
      ...
      if not df_p.empty:
        ...
        with col_p2:
          ...
          if st.button("🗑️ Excluir Item"):
            cursor.execute("DELETE FROM pecas WHERE id = ?", (peca_para_excluir,))
            conn.commit()
